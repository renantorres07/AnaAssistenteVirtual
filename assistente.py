...
def clima(cidade):
    token = "f778d371086ce23ccf716ccdf1f26b70"
...
def tradutor(traducao):
    if traducao == 'inglês':
        ...
        traduz = Translator(from_lang="pt-br", to_lang='english')
        cria_audio("traducao.mp3", "O que você gostaria de traduzir para o portugês ?")
        mensagem = monitora_audio()
        traduzido = traduz.translate(mensagem)
...
def executa_comandos(mensagem):
    ...
    # Abrir programas do computador 
        ...
        os.startfile("C:\Users\renan.torres\Desktop\Visual Studio Code.lnk")
    ...
    # Lembrete
        ...
        os.system(f'c: && C:/Program Files/Conceptworld/Notezilla && Notezilla.exe /CreateNewNote "{lembrete}"')
    elif 'mostrar' in mensagem and 'lembrete' in mensagem:
        os.system('c: && C:/Program Files/ConceptWorld/Notezilla && Notezilla.exe /BringNotesOnTop')

def main():
    cria_audio("ola.mp3", "Olá, eu sou a Ana, sua assistente virtual! Como posso ajudar?")
    while True:
        monitora_audio()
